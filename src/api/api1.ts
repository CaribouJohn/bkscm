...
// import { BkBaseApi } from '../bk-base';
// import { PickRemoteSourceOptions } from './bk-base';
...
export class ApiRepositoryState implements RepositoryState {
  ...
  readonly onDidChange: Event<void> = this._repository.onDidRunBkStatus;
...
export class ApiBk implements Bk {

  get path(): string { return this._model.bk.path; }
...
export class ApiImpl implements API {
  ...
  readonly bk = new ApiBk(this._model);
  ...
  toBkUri(uri: Uri, ref: string): Uri {
    return toBkUri(uri, ref);
  }
  ...
  async init(root: Uri): Promise<Repository | null> {
    ...
    await this._model.bk.init(path);
  ...
  registerRemoteSourceProvider(provider: RemoteSourceProvider): Disposable {
    ...
    //disposables.push(BkBaseApi.getAPI().registerRemoteSourceProvider(provider));
...
export function registerAPICommands(extension: BkExtensionImpl): Disposable {
  const disposables: Disposable[] = [];

  disposables.push(commands.registerCommand('bk.api.getRepositories', () => {
    const api = extension.getAPI(1);
    return api.repositories.map(r => r.rootUri.toString());
  }));

  disposables.push(commands.registerCommand('bk.api.getRepositoryState', (uri: string) => {
    const api = extension.getAPI(1);
  ...
  // disposables.push(commands.registerCommand('bk.api.getRemoteSources', (opts?: PickRemoteSourceOptions) => {
  //   return commands.executeCommand('bk-base.api.getRemoteSources', opts);
  // }));
