...
export class BkTimelineItem extends TimelineItem {
	static is(item: TimelineItem): item is BkTimelineItem {
		return item instanceof BkTimelineItem;
	}
	...
	setItemDetails(author: string, email: string | undefined, date: string, message: string): void {
		...
		if (email) {
			const emailTitle = localize('bk.timeline.email', "Email");
...
export class BkTimelineProvider implements TimelineProvider {
	...
	readonly id = 'bk-history';
	readonly label = localize('bk.timeline.source', 'Bk History');
	...
	async provideTimeline(uri: Uri, options: TimelineOptions, _token: CancellationToken): Promise<Timeline> {
		// console.log(`BkTimelineProvider.provideTimeline: uri=${uri} state=${this._model.state}`);
		...
		if (this.repo?.root !== repo.root) {
			...
			this.repoDisposable = Disposable.from(
				...
				repo.onDidRunBkStatus(() => this.onRepositoryStatusChanged(repo))
			);
		}

		const config = workspace.getConfiguration('bk.timeline');
		...
		if (options.limit !== undefined && typeof options.limit !== 'number') {
			try {
				const result = await this.model.bk.exec(repo.root, ['rev-list', '--count', `${options.limit.id}..`, '--', uri.fsPath]);
				if (!result.exitCode) {
		...
		const openComparison = localize('bk.timeline.openComparison', "Open Comparison");

		const items = commits.map<BkTimelineItem>((c, i) => {
			const date = dateType === 'authored' ? c.authorDate : c.commitDate;

			const message = c.message; //emojify(c.message);

			const item = new BkTimelineItem(c.hash, commits[i + 1]?.hash ?? `${c.hash}^`, message, date?.getTime() ?? 0, c.hash, 'bk:file:commit');
			item.iconPath = new ThemeIcon('bk-commit');
			if (showAuthor) {
		...
		if (options.cursor === undefined) {
			const you = localize('bk.timeline.you', 'You');

			const index = repo.indexGroup.resourceStates.find(r => r.resourceUri.fsPath === uri.fsPath);
			if (index) {
				const date = this.repoStatusDate ?? new Date();

				const item = new BkTimelineItem('~', 'HEAD', localize('bk.timeline.checkindChanges', 'Checked In Changes'), date.getTime(), 'index', 'bk:file:index');
			...
			if (showUncommitted) {
				...
				if (working) {
					...
					const item = new BkTimelineItem('', index ? '~' : 'HEAD', localize('bk.timeline.uncommitedChanges', 'Uncommitted Changes'), date.getTime(), 'working', 'bk:file:working');
	...
	private ensureProviderRegistration() {
		if (this.providerDisposable === undefined) {
			this.providerDisposable = workspace.registerTimelineProvider(['file', 'bk', 'vscode-remote', 'bklens-bk', 'vscode-local-history'], this);
		}
	}

	private onConfigurationChanged(e: ConfigurationChangeEvent) {
		if (e.affectsConfiguration('bk.timeline.date') || e.affectsConfiguration('bk.timeline.showAuthor') || e.affectsConfiguration('bk.timeline.showUncommitted')) {
			this.fireChanged();
		}
	}

	private onRepositoriesChanged(_repo: Repository) {
		// console.log(`BkTimelineProvider.onRepositoriesChanged`);
	...
	private onRepositoryChanged(_repo: Repository, _uri: Uri) {
		// console.log(`BkTimelineProvider.onRepositoryChanged: uri=${uri.toString(true)}`);

		this.fireChanged();
	}

	private onRepositoryStatusChanged(_repo: Repository) {
		// console.log(`BkTimelineProvider.onRepositoryStatusChanged`);

		// This is less than ideal, but for now just save the last time a status was run and use that as the timestamp for checkind items
		this.repoStatusDate = new Date();
