...
class CheckoutItem implements QuickPickItem {
	...
	get label(): string { return `${this.repository.isBranchProtected(this.ref.name ?? '') ? '$(lock)' : '$(bk-branch)'} ${this.ref.name || this.shortCommit}`; }
...
export class CommandCenter {
	...
	constructor(
		...
		this.disposables.push(workspace.registerTextDocumentContentProvider('bk-output', this.commandErrors));
	}
	...
	@command('bk.refresh', { repository: true })
	async refresh(repository: Repository): Promise<void> {
		await repository.status();
	}

	@command('bk.openResource')
	...
	@command('bk.openAllChanges', { repository: true })
	...
	async openMergeEditor(uri: unknown) {
		...
		try {
			...
			current.description = '$(bk-commit) ' + head.hash.substring(0, 7);

			// theirs
			incoming.detail = rebaseOrMergeHead.refNames.join(', ');
			incoming.description = '$(bk-commit) ' + rebaseOrMergeHead.hash.substring(0, 7);
...
//			this.telemetryReporter.sendTelemetryEvent('clone', { outcome: 'no_URL' });
		...
		url = url.trim().replace(/^bk\s+clone\s+/, '');
		...
		try {
			const opts = {
				...
				title: localize('cloning', "Cloning bk repository '{0}'...", url),
				cancellable: true
			};

			const repositoryPath = await window.withProgress(
				opts,
				(progress, token) => this.bk.clone(url!, { parentPath: parentPath!, progress, recursive: options.recursive }, token)
			);
	...
	async init(skipFolderPrompt = false): Promise<void> {
		...
		if (!repositoryPath) {
			...
			if (homeUri.toString().startsWith(uri.toString())) {
				...
				const answer = await window.showWarningMessage(localize('are you sure', "This will create a Bk repository in '{0}'. Are you sure you want to continue?", uri.fsPath), yes);
		...
		await this.bk.init(repositoryPath);
	...
	@command('bk.openRepository', { repository: false })
	...
	@command('bk.close', { repository: true })
	...
	async openFile(arg?: Resource | Uri, ...resourceStates: SourceControlResourceState[]): Promise<void> {
		...
		if (arg instanceof Uri) {
			if (isBkUri(arg)) {
				uris = [Uri.file(fromBkUri(arg).path)];
	...
	@command('bk.rename', { repository: true })
	...
	@command('bk.cleanAll', { repository: true })
	...
	@command('bk.cleanAllTracked', { repository: true })
	...
	@command('bk.cleanAllUntracked', { repository: true })
	...
	private async smartCommit(
		...
		if (opts.noVerify) {
			if (!config.get<boolean>('allowNoVerifyCommit')) {
				await window.showErrorMessage(localize('no verify commit not allowed', "Commits without verification are not allowed, please enable them with the 'bk.allowNoVerifyCommit' setting."));
				return false;
			}
	...
	@command('bk.commit', { repository: true })
	...
	async commitMessageDiscard(arg?: Uri): Promise<void> {
		...
		const editors = window.visibleTextEditors
			.filter(e => e.document.languageId === 'bk-commit' && e.document.uri.toString() === arg.toString());
	...
	@command('bk.commitAllNoVerify', { repository: true })
	...
	@command('bk.commitAllSignedNoVerify', { repository: true })
	...
	@command('bk.commitAllAmendNoVerify', { repository: true })
	...
	@command('bk.branch', { repository: true })
	async branch(repository: Repository): Promise<void> {
		await this._branch(repository);
	}

	@command('bk.branchFrom', { repository: true })
	...
	@command('bk.deleteBranch', { repository: true })
	...
	@command('bk.renameBranch', { repository: true })
		...
		try {
			...
			switch (err.bkErrorCode) {
				case BkErrorCodes.InvalidBranchName:
					...
					return;
				case BkErrorCodes.BranchAlreadyExists:
	...
	@command('bk.merge', { repository: true })
	...
	@command('bk.rebase', { repository: true })
	...
	@command('bk.createTag', { repository: true })
	...
	@command('bk.deleteTag', { repository: true })
	...
	@command('bk.fetch', { repository: true })
	...
	@command('bk.fetchPrune', { repository: true })
	...
	@command('bk.fetchAll', { repository: true })
	...
	@command('bk.pullFrom', { repository: true })
	...
	@command('bk.pull', { repository: true })
	...
	@command('bk.pullRebase', { repository: true })
	...
	private async _push(repository: Repository, pushOptions: PushOptions) {
		...
		if (pushOptions.forcePush) {
			if (!config.get<boolean>('allowForcePush')) {
				await window.showErrorMessage(localize('force push not allowed', "Force push is not allowed, please enable it with the 'bk.allowForcePush' setting."));
				return;
			}
	...
	@command('bk.cherryPick', { repository: true })
	...
	@command('bk.pushTo', { repository: true })
	...
	@command('bk.pushToForce', { repository: true })
	...
	@command('bk.removeRemote', { repository: true })
	...
	@command('bk.sync', { repository: true })
	...
	@command('bk.syncRebase', { repository: true })
	...
	@command('bk.publish', { repository: true })
	...
	@command('bk.stashPopLatest', { repository: true })
	...
	@command('bk.stashApply', { repository: true })
	...
	@command('bk.stashApplyLatest', { repository: true })
	...
	@command('bk.stashDrop', { repository: true })
	...
	@command('bk.stashDropAll', { repository: true })
	...
	// @command('bk.timeline.copyCommitMessage', { repository: false })
	...
	// private _selectedForCompare: { uri: Uri; item: BkTimelineItem } | undefined;

	// @command('bk.timeline.selectForCompare', { repository: false })
	// async timelineSelectForCompare(item: TimelineItem, uri: Uri | undefined, _source: string) {
	// 	if (!BkTimelineItem.is(item) || !uri) {
	// 		return;
	// 	}

	// 	this._selectedForCompare = { uri, item };
	// 	await commands.executeCommand('setContext', 'bk.timeline.selectedForCompare', true);
	// }

	// @command('bk.timeline.compareWithSelected', { repository: false })
	// async timelineCompareWithSelected(item: TimelineItem, uri: Uri | undefined, _source: string) {
	// 	if (!BkTimelineItem.is(item) || !uri || !this._selectedForCompare || uri.toString() !== this._selectedForCompare.uri.toString()) {
	// 		return;
	// 	}
	...
	// 		leftTitle = localize('bk.title.workingTree', '{0} (Working Tree)', basename);
	// 	}
	// 	else if (selected.previousRef === 'HEAD' && selected.ref === '~') {
	// 		leftTitle = localize('bk.title.index', '{0} (Index)', basename);
	// 	} else {
	// 		leftTitle = localize('bk.title.ref', '{0} ({1})', basename, selected.shortRef);
	// 	}

	// 	let rightTitle;
	// 	if ((item.previousRef === 'HEAD' || item.previousRef === '~') && item.ref === '') {
	// 		rightTitle = localize('bk.title.workingTree', '{0} (Working Tree)', basename);
	// 	}
	// 	else if (item.previousRef === 'HEAD' && item.ref === '~') {
	// 		rightTitle = localize('bk.title.index', '{0} (Index)', basename);
	// 	} else {
	// 		rightTitle = localize('bk.title.ref', '{0} ({1})', basename, item.shortRef);
	// 	}


	// 	const title = localize('bk.title.diff', '{0} ↔ {1}', leftTitle, rightTitle);
	// 	await commands.executeCommand('vscode.diff', selected.ref === '' ? uri : toBkUri(uri, selected.ref), item.ref === '' ? uri : toBkUri(uri, item.ref), title);
	// }

	// @command('bk.rebaseAbort', { repository: true })
	...
	@command('bk.closeAllDiffEditors', { repository: true })
	closeDiffEditors(repository: Repository): void {
		repository.closeDiffEditors(undefined, undefined, true);
	}

	private createCommand(_id: string, key: string, method: Function, options: ScmCommandOptions): (...args: any[]) => any {
		const result = (...args: any[]) => {
			...
			*/
			//this.telemetryReporter.sendTelemetryEvent('bk.command', { command: id });
			...
			return result.catch(async err => {
				...
				const openOutputChannelChoice = localize('open bk log', "Open Bk Log");
				...
				if (err.stderr) {
					choices.set(showCommandOutputChoice, async () => {
						...
						const uri = Uri.parse(`bk-output:/bk-error-${timestamp}`);

						let command = 'bk';

						if (err.bkArgs) {
							command = `${command} ${err.bkArgs.join(' ')}`;
						} else if (err.bkCommand) {
							command = `${command} ${err.bkCommand}`;
						}
				...
				switch (err.bkErrorCode) {
					case BkErrorCodes.DirtyWorkTree:
						message = localize('clean repo', "Please clean your repository working tree before checkout.");
						break;
					case BkErrorCodes.PushRejected:
						message = localize('cant push', "Can't push refs to remote. Try running 'Pull' first to integrate your changes.");
						break;
					case BkErrorCodes.Conflict:
						message = localize('merge conflicts', "There are merge conflicts. Resolve them before committing.");
						type = 'warning';
						options.modal = false;
						break;
					case BkErrorCodes.StashConflict:
						message = localize('stash merge conflicts', "There were merge conflicts while applying the stash.");
						type = 'warning';
						options.modal = false;
						break;
					case BkErrorCodes.AuthenticationFailed: {
						const regex = /Authentication failed for '(.*)'/i;
						const match = regex.exec(err.stderr || String(err));

						message = match
							? localize('auth failed specific', "Failed to authenticate to bk remote:\n\n{0}", match[1])
							: localize('auth failed', "Failed to authenticate to bk remote.");
						break;
					}
					case BkErrorCodes.NoUserNameConfigured:
					case BkErrorCodes.NoUserEmailConfigured:
						message = localize('missing user info', "Make sure you configure your 'user.name' and 'user.email' in bk.");
						choices.set(localize('learn more', "Learn More"), () => commands.executeCommand('vscode.open', Uri.parse('https://aka.ms/vscode-setup-bk')));
						break;
					case BkErrorCodes.EmptyCommitMessage:
					...
					default: {
						...
						message = hint
							? localize('bk error details', "Bk: {0}", hint)
							: localize('bk error', "Bk error");
	...
	private getSCMResource(uri?: Uri): Resource | undefined {
		...
		this.outputChannelLogger.logDebug(`bk.getSCMResource.uri ${uri && uri.toString()}`);
	...
	private async runByRepository<T>(arg: Uri | Uri[], fn: (repository: Repository, resources: any) => Promise<T>): Promise<T[]> {
		...
		const groups = resources.reduce((result, resource) => {
			...
			if (!repository) {
				console.warn('Could not find bk repository for ', resource);
				return result;
			}
