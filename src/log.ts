...
export class OutputChannelLogger {
	...
	set currentLogLevel(value: LogLevel) {
		...
		this.log(localize('bkLogLevel', "Log level: {0}", LogLevel[value]));
	}
	...
	constructor() {
		// Output channel
		this._outputChannel = window.createOutputChannel('Bk');
		commands.registerCommand('bk.showOutput', () => this.showOutputChannel());
		this._disposables.push(this._outputChannel);

		this._disposables.push(workspace.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('bk.logLevel')) {
