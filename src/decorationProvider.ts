...
import { Repository, BkResourceGroup } from './repository';
import { Model } from './model';
import { debounce } from './decorators';
import { filterEvent, dispose, anyEvent, fireEvent, PromiseSource } from './util';
import { BkErrorCodes, Status } from './api/bk';

class BkIgnoreDecorationProvider implements FileDecorationProvider {

	private static Decoration: FileDecoration = { color: new ThemeColor('bkDecoration.ignoredResourceForeground') };
	...
	constructor(private model: Model) {
		this.onDidChangeFileDecorations = fireEvent(anyEvent<any>(
			filterEvent(workspace.onDidSaveTextDocument, e => /\.bkignore$|\.bk\/info\/exclude$/.test(e.uri.path)),
		...
		for (const [, item] of queue) {
			...
			item.repository.checkIgnore(paths).then(ignoreSet => {
				for (const [path, promiseSource] of item.queue.entries()) {
					promiseSource.resolve(ignoreSet.has(path) ? BkIgnoreDecorationProvider.Decoration : undefined);
				}
			}, err => {
				if (err.bkErrorCode !== BkErrorCodes.IsInSubmodule) {
					console.error(err);
				}
...
class BkDecorationProvider implements FileDecorationProvider {

	// private static SubmoduleDecorationData: FileDecoration = {
	// 	tooltip: 'Submodule',
	// 	badge: 'S',
	// 	color: new ThemeColor('bkDecoration.submoduleResourceForeground')
	// };
	...
	constructor(private repository: Repository) {
		this.disposables.push(
			...
			repository.onDidRunBkStatus(this.onDidRunBkStatus, this)
		);
	}

	private onDidRunBkStatus(): void {
	...
	private collectDecorationData(group: BkResourceGroup, bucket: Map<string, FileDecoration>): void {
	...
	private collectSubmoduleDecorationData(_bucket: Map<string, FileDecoration>): void {
		// for (const submodule of this.repository.submodules) {
		// 	bucket.set(Uri.file(path.join(this.repository.root, submodule.path)).toString(), BkDecorationProvider.SubmoduleDecorationData);
		// }
	}
...
export class BkDecorations {
	...
	constructor(private model: Model) {
		this.disposables.push(new BkIgnoreDecorationProvider(model));

		const onEnablementChange = filterEvent(workspace.onDidChangeConfiguration, e => e.affectsConfiguration('bk.decorations.enabled'));
		onEnablementChange(this.update, this, this.disposables);
		this.update();
	}

	private update(): void {
		const enabled = workspace.getConfiguration('bk').get('decorations.enabled');
	...
	private onDidOpenRepository(repository: Repository): void {
		const provider = new BkDecorationProvider(repository);
