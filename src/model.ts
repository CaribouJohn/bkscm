...
import { dispose, anyEvent, filterEvent, isDescendant, pathEquals, toDisposable, eventToPromise, EmptyDisposable } from './util';
import { Bk } from './bitkeeper';
import * as path from 'path';
import * as fs from 'fs';
import * as nls from 'vscode-nls';
import { fromBkUri } from './uri';
import { APIState as State, CredentialsProvider, PushErrorHandler, PublishEvent, RemoteSourcePublisher, PostCommitCommandsProvider } from './api/bk';
...
export class Model  {
	...
	private possibleBkRepositoryPaths = new Set<string>();
	...
	setState(state: State): void {
		...
		commands.executeCommand('setContext', 'bk.state', state);
	}
	...
	constructor(readonly bk: Bk,  private globalState: Memento, private outputChannelLogger: OutputChannelLogger) {
		...
		const onBkRepositoryChange = filterEvent(onWorkspaceChange, uri => /\/\.bk/.test(uri.path));
		const onPossibleBkRepositoryChange = filterEvent(onBkRepositoryChange, uri => !this.getRepository(uri));
		onPossibleBkRepositoryChange(this.onPossibleBkRepositoryChange, this, this.disposables);
	...
	private async doInitialScan(): Promise<void> {
		...
		*/
		// this.telemetryReporter.sendTelemetryEvent('bk.repositoryInitialScan', { autoRepositoryDetection: String(autoRepositoryDetection) }, { repositoryCount: this.openRepositories.length });
	}

	/**
	 * Scans each workspace folder, looking for bk repositories. By
	 * default it scans one level deep but that can be changed using
	 * the bk.repositoryScanMaxDepth setting.
	 */
	private async scanWorkspaceFolders(): Promise<void> {
		...
		await Promise.all((workspace.workspaceFolders || []).map(async folder => {
			...
			const repositoryScanMaxDepth = (workspace.isTrusted ? workspace.getConfiguration('bk', folder.uri) : config).get<number>('repositoryScanMaxDepth', 1);
			const repositoryScanIgnoredFolders = (workspace.isTrusted ? workspace.getConfiguration('bk', folder.uri) : config).get<string[]>('repositoryScanIgnoredFolders', []);
			...
			const scanPaths = (workspace.isTrusted ? workspace.getConfiguration('bk', folder.uri) : config).get<string[]>('scanRepositories') || [];
			...
			for (const scanPath of scanPaths) {
				if (scanPath === '.bk') {
					this.outputChannelLogger.logTrace('[swsf] \'.bk\' not supported in \'bk.scanRepositories\' setting.');
					continue;
				}

				if (path.isAbsolute(scanPath)) {
					const notSupportedMessage = localize('not supported', "Absolute paths not supported in 'bk.scanRepositories' setting.");
		...
		while (foldersToTravers.length > 0) {
			...
			if (currentFolder.depth < maxDepth || maxDepth === -1) {
				...
				const childrenFolders = children
					.filter(dirent =>
						dirent.isDirectory() && dirent.name !== '.bk' &&
	...
	private onPossibleBkRepositoryChange(uri: Uri): void {
		const config = workspace.getConfiguration('bk');
		const autoRepositoryDetection = config.get<boolean | 'subFolders' | 'openEditors'>('autoRepositoryDetection');
		...
		this.eventuallyScanPossibleBkRepository(uri.fsPath.replace(/\.bk.*$/, ''));
	}

	private eventuallyScanPossibleBkRepository(path: string) {
		this.possibleBkRepositoryPaths.add(path);
		this.eventuallyScanPossibleBkRepositories();
	}

	@debounce(500)
	private eventuallyScanPossibleBkRepositories(): void {
		for (const path of this.possibleBkRepositoryPaths) {
			this.openRepository(path);
		}

		this.possibleBkRepositoryPaths.clear();
	}
	...
	private onDidChangeConfiguration(): void {
		const possibleRepositoryFolders = (workspace.workspaceFolders || [])
			.filter(folder => workspace.getConfiguration('bk', folder.uri).get<boolean>('enabled') === true)
			.filter(folder => !this.getOpenRepository(folder.uri));

		const openRepositoriesToDispose = this.openRepositories
			.map(repository => ({ repository, root: Uri.file(repository.repository.root) }))
			.filter(({ root }) => workspace.getConfiguration('bk', root).get<boolean>('enabled') !== true)
	...
	async openRepository(repoPath: string): Promise<void> {
		...
		const config = workspace.getConfiguration('bk', Uri.file(repoPath));
		const enabled = config.get<boolean>('enabled') === true;

		if (!enabled) {
			this.outputChannelLogger.logTrace('Bk is not enabled');
			return;
		}
		...
		try {
			...
			// https://bkhub.com/microsoft/vscode/issues/33498
			...
			// On Window, opening a bk repository from the root of the HOMEDRIVE poses a security risk.
			// We will only a open bk repository from the root of the HOMEDRIVE if the user explicitly
			...
			if (process.platform === 'win32' && process.env.HOMEDRIVE && pathEquals(`${process.env.HOMEDRIVE}\\`, repositoryRoot)) {
				...
				if (!isRepoInWorkspaceFolders) {
					if (this.showRepoOnHomeDriveRootWarning) {
						window.showWarningMessage(localize('repoOnHomeDriveRootWarning', "Unable to automatically open the bk repository at '{0}'. To open that bk repository, open it directly as a folder in VS Code.", repositoryRoot));
			...
			const dotBk = await this.bk.getRepositoryDotBk(repositoryRoot);
			const repository = new Repository(this.bk.open(repositoryRoot, dotBk), this.globalState, this.outputChannelLogger);
	...
	private open(repository: Repository): void {
		...
		const shouldDetectSubmodules = workspace
			.getConfiguration('bk', Uri.file(repository.root))
		...
		const checkForSubmodules = () => {
			...
			// 	.forEach(p => this.eventuallyScanPossibleBkRepository(p));
		};

		const statusListener = repository.onDidRunBkStatus(checkForSubmodules);
		checkForSubmodules();
	...
	private getOpenRepository(hint: any): OpenRepository | undefined {
		...
		if (hint instanceof Uri) {
			...
			if (hint.scheme === 'bk') {
				resourcePath = fromBkUri(hint).path;
			} else {
	...
	getRepositoryForSubmodule(_submoduleUri: Uri): Repository | undefined {
		for (const {} of this.repositories) {
	...
	registerCredentialsProvider(_provider: CredentialsProvider): Disposable {
