...
export class Resource implements SourceControlResourceState {
	...
	private getIconPath(theme: string): Uri {
		switch (this.type) {
			...
			default: throw new Error('Unknown bk status: ' + this.type);
		}
	}
	...
	get letter(): string {
		switch (this.type) {
			...
			default:
				throw new Error('Unknown bk status: ' + this.type);
		}
	}
	...
	get color(): ThemeColor {
		switch (this.type) {
			...
			case Status.INDEX_ADDED:
			case Status.INTENT_TO_ADD:
				return new ThemeColor('bkDecoration.addedResourceForeground');
			case Status.INDEX_COPIED:
			case Status.INDEX_RENAMED:
				return new ThemeColor('bkDecoration.renamedResourceForeground');
			case Status.UNTRACKED:
				return new ThemeColor('bkDecoration.untrackedResourceForeground');
			case Status.IGNORED:
				return new ThemeColor('bkDecoration.ignoredResourceForeground');
			case Status.BOTH_DELETED:
			case Status.ADDED_BY_US:
			case Status.DELETED_BY_THEM:
			case Status.ADDED_BY_THEM:
			...
			case Status.BOTH_ADDED:
			case Status.BOTH_MODIFIED:
				return new ThemeColor('bkDecoration.conflictingResourceForeground');
			default:
				throw new Error('Unknown bk status: ' + this.type);
		}
	}
...
export interface BkResourceGroup extends SourceControlResourceGroup {
...
class ProgressManager {
	...
	constructor(private repository: Repository) {
		const onDidChange = filterEvent(workspace.onDidChangeConfiguration, e => e.affectsConfiguration('bk', Uri.file(this.repository.root)));
	...
	private updateEnablement(): void {
		const config = workspace.getConfiguration('bk', Uri.file(this.repository.root));
...
class FileEventLogger {
	...
	constructor(
		...
		private onDotBkFileChange: Event<Uri>,
	...
	private onDidChangeLogLevel(level: LogLevel): void {
		...
		this.eventDisposable = combinedDisposable([
			...
			this.onDotBkFileChange(uri => this.outputChannelLogger.logDebug(`[.bk] Change: ${uri.fsPath}`))
		]);
	}
...
class DotBkWatcher implements IFileWatcher {
	...
	constructor(
		...
		const rootWatcher = watch(repository.dotBk.path);
		this.disposables.push(rootWatcher);

		// Ignore changes to the "index.lock" file, and watchman fsmonitor hook (https://bk-scm.com/docs/bkhooks#_fsmonitor_watchman) cookie files.
		// Watchman creates a cookie file inside the bk directory whenever a query is run (https://facebook.bkhub.io/watchman/docs/cookies.html).
		const filteredRootWatcher = filterEvent(rootWatcher.event, uri => uri.scheme === 'file' && !/\/\.bk(\/index\.lock)?$|\/\.watchman-cookie-/.test(uri.path));
		this.event = anyEvent(filteredRootWatcher, this.emitter.event);

		repository.onDidRunBkStatus(this.updateTransientWatchers, this, this.disposables);
	...
	private updateTransientWatchers() {
		...
		const upstreamPath = path.join(this.repository.dotBk.commonPath ?? this.repository.dotBk.path, 'refs', 'remotes', remote, name);
...
class ResourceCommandResolver {
	...
	resolveChangeCommand(resource: Resource): Command {
		...
		if (!resource.leftUri) {
			...
			if (resource.rightUri && workspace.getConfiguration('bk').get<boolean>('mergeEditor', false) && (bothModified || resource.type === Status.BOTH_ADDED)) {
				return {
	...
	getResources(resource: Resource): [Uri | undefined, Uri | undefined] {
		...
		// 		return [undefined, toBkUri(resource.resourceUri, resource.resourceGroupType === ResourceGroupType.Index ? 'index' : 'wt', { submoduleOf: this.repository.root })];
		// 	}
		// }
	...
	private getLeftResource(resource: Resource): Uri | undefined {
		switch (resource.type) {
			...
			case Status.INDEX_RENAMED:
			case Status.INDEX_ADDED:
				return toBkUri(resource.original, 'HEAD');

			case Status.MODIFIED:
			case Status.UNTRACKED:
				return toBkUri(resource.resourceUri, '~');

			case Status.DELETED_BY_US:
			case Status.DELETED_BY_THEM:
				return toBkUri(resource.resourceUri, '~1');
		}
		return undefined;
	}
	...
	private getRightResource(resource: Resource): Uri | undefined {
		switch (resource.type) {
			...
			case Status.INDEX_DELETED:
			case Status.DELETED:
				return toBkUri(resource.resourceUri, 'HEAD');

			case Status.DELETED_BY_US:
				return toBkUri(resource.resourceUri, '~3');

			case Status.DELETED_BY_THEM:
				return toBkUri(resource.resourceUri, '~2');
	...
	private getTitle(resource: Resource): string {
		...
		switch (resource.type) {
			...
			case Status.INDEX_RENAMED:
			case Status.INDEX_ADDED:
				return localize('bk.title.index', '{0} (Index)', basename);

			case Status.MODIFIED:
			case Status.BOTH_ADDED:
			case Status.BOTH_MODIFIED:
				return localize('bk.title.workingTree', '{0} (Working Tree)', basename);

			case Status.INDEX_DELETED:
			case Status.DELETED:
				return localize('bk.title.deleted', '{0} (Deleted)', basename);

			case Status.DELETED_BY_US:
				return localize('bk.title.theirs', '{0} (Theirs)', basename);

			case Status.DELETED_BY_THEM:
				return localize('bk.title.ours', '{0} (Ours)', basename);

			case Status.UNTRACKED:
				return localize('bk.title.untracked', '{0} (Untracked)', basename);
...
export class Repository implements Disposable {
	...
	readonly onDidRunBkStatus: Event<void> = this._onDidChangeStatus.event;
	...
	get mergeGroup(): BkResourceGroup { return this._mergeGroup as BkResourceGroup; }

	private _indexGroup: SourceControlResourceGroup;
	get indexGroup(): BkResourceGroup { return this._indexGroup as BkResourceGroup; }

	private _workingTreeGroup: SourceControlResourceGroup;
	get workingTreeGroup(): BkResourceGroup { return this._workingTreeGroup as BkResourceGroup; }

	private _untrackedGroup: SourceControlResourceGroup;
	get untrackedGroup(): BkResourceGroup { return this._untrackedGroup as BkResourceGroup; }
	...
	// 	commands.executeCommand('setContext', 'bkRebaseInProgress', !!this._rebaseCommit);
	// }
	...
	constructor(
		...
		const onRepositoryWorkingTreeFileChange = filterEvent(onRepositoryFileChange, uri => !/\.bk($|\/)/.test(relativePath(repository.root, uri.fsPath)));

		let onRepositoryDotBkFileChange: Event<Uri>;
		...
		// FS changes should trigger `bk status`:
		// 	- any change inside the repository working tree
		//	- any change whithin the first level of the `.bk` folder, except the folder itself and `index.lock`
		const onFileChange = anyEvent(onRepositoryWorkingTreeFileChange, onRepositoryDotBkFileChange);
		onFileChange(this.onFileChange, this, this.disposables);

		// Relevate repository changes should trigger virtual document change events
		onRepositoryDotBkFileChange(this._onDidChangeRepository.fire, this._onDidChangeRepository, this.disposables);

		this.disposables.push(new FileEventLogger(onRepositoryWorkingTreeFileChange, onRepositoryDotBkFileChange, outputChannelLogger));

		const root = Uri.file(repository.root);
		this._sourceControl = scm.createSourceControl('bk', 'Bk', root);

		this._sourceControl.acceptInputCommand = { command: 'bk.commit', title: localize('commit', "Commit"), arguments: [this._sourceControl] };
		...
		workspace.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('bk.mergeEditor')) {
		...
		filterEvent(workspace.onDidChangeConfiguration, e =>
			e.affectsConfiguration('bk.branchProtection', root)
			|| e.affectsConfiguration('bk.branchSortOrder', root)
			|| e.affectsConfiguration('bk.untrackedChanges', root)
			|| e.affectsConfiguration('bk.ignoreSubmodules', root)
			|| e.affectsConfiguration('bk.openDiffOnClick', root)
			|| e.affectsConfiguration('bk.showActionButton', root)
		...
		const onConfigListenerForInputBoxVisibility = filterEvent(workspace.onDidChangeConfiguration, e => e.affectsConfiguration('bk.showCommitInput', root));
		...
		// https://bkhub.com/microsoft/vscode/issues/39039
		const onSuccessfulPush = filterEvent(this.onDidRunOperation, e => e.operation === Operation.Push && !e.error);
		onSuccessfulPush(() => {
			const bkConfig = workspace.getConfiguration('bk');

			if (bkConfig.get<boolean>('showPushSuccessNotification')) {
				window.showInformationMessage(localize('push success', "Successfully pushed."));
			}
		}, null, this.disposables);

		const onDidChangeBranchProtection = filterEvent(workspace.onDidChangeConfiguration, e => e.affectsConfiguration('bk.branchProtection', root));
		...
		const onDidChangeCountBadge = filterEvent(workspace.onDidChangeConfiguration, e => e.affectsConfiguration('bk.countBadge', root));
	...
	provideOriginalResource(uri: Uri): Uri | undefined {
		...
		return toBkUri(uri, '', { replaceFileExtension: true });
	}
	...
	closeDiffEditors(indexResources: string[] | undefined, workingTreeResources: string[] | undefined, ignoreSetting: boolean = false): void {
		...
		for (const tab of window.tabGroups.all.map(g => g.tabs).flat()) {
			...
			if (input instanceof TabInputTextDiff || input instanceof TabInputNotebookDiff) {
				if (input.modified.scheme === 'bk' && (indexResources === undefined || indexResources.some(r => pathEquals(r, input.modified.fsPath)))) {
					// Index
					diffEditorTabsToClose.push(tab);
				}
				if (input.modified.scheme === 'file' && input.original.scheme === 'bk' && (workingTreeResources === undefined || workingTreeResources.some(r => pathEquals(r, input.modified.fsPath)))) {
					// Working Tree
					diffEditorTabsToClose.push(tab);
				}
			}
	...
	async ignore(files: Uri[]): Promise<void> {
		return await this.run(Operation.Ignore, async () => {
			const ignoreFile = `${this.repository.root}${path.sep}.bkignore`;
	...
	checkIgnore(filePaths: string[]): Promise<Set<string>> {
		return this.run(Operation.CheckIgnore, () => {
			return new Promise<Set<string>>((resolve, reject) => {
				...
				// https://bk-scm.com/docs/bk-check-ignore#bk-check-ignore--z
				...
				const onExit = (exitCode: number) => {
					if (exitCode === 1) {
						...
						if (/ is in submodule /.test(stderr)) {
							reject(new BkError({ stdout: data, stderr, exitCode, bkErrorCode: BkErrorCodes.IsInSubmodule }));
						} else {
							reject(new BkError({ stdout: data, stderr, exitCode }));
						}
					}
				};
	...
	// Parses output of `bk check-ignore -v -z` and returns only those paths
	// that are actually ignored by bk.
	// Matches to a negative pattern (starting with '!') are filtered out.
	// See also https://bk-scm.com/docs/bk-check-ignore#_output.
	...
	private async updateModelState(): Promise<void> {
		const scopedConfig = workspace.getConfiguration('bk', Uri.file(this.repository.root));
		...
		const { status, didHitLimit } = await this.repository.getStatus({ limit, ignoreSubmodules, untrackedChanges });
		...
		if (didHitLimit && !shouldIgnore && !this.didWarnAboutLimit) {
			...
			const bkWarn = localize('huge', "The bk repository at '{0}' has too many active changes, only a subset of Bk features will be enabled.", this.repository.root);
			...
			if (knownHugeFolderPaths.length > 0) {
				...
				const addKnown = localize('add known', "Would you like to add '{0}' to .bkignore?", folderName);
				const yes = { title: localize('yes', "Yes") };
				const no = { title: localize('no', "No") };

				const result = await window.showWarningMessage(`${bkWarn} ${addKnown}`, yes, no, neverAgain);
				if (result === yes) {
				...
				const result = await window.showWarningMessage(bkWarn, ok, neverAgain);
				if (result === neverAgain) {
					config.update('ignoreLimitWarning', true, false);
				}
		...
		if (totalTime > 5000) {
			/* __GDPR__
				"statusSlow" : {
					"owner": "dibkarald",
					"comment": "Reports when bk status is slower than 5s",
					"expiration": "1.73",
					"ignoreSubmodules": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Setting indicating whether submodules are ignored" },
					"didHitLimit": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "Total number of status entries" },
					"didWarnAboutLimit": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "True when the user was warned about slow bk status" },
					"statusLength": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Total number of status entries" },
					"totalTime": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Total number of ms the operation took" }
				}
			*/
		...
		commands.executeCommand('setContext', 'bk.mergeChanges', merge.map(item => item.resourceUri.toString()));
